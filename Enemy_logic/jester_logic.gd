extends RefCounted
class_name JesterLogic

## Мистический шут: если нет баффа «Бить наверняка» — 50% применить его. Иначе случайный навык.

static func get_decision(monster: Combatant, heroes: Array) -> Dictionary:
	var sure_ab := _find_by_name(monster, "Бить наверняка")
	if sure_ab != null and _is_usable_from_position(sure_ab, monster.position_index):

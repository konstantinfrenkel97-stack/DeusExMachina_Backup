...
static func get_stance_effect_description(stance_effect_type: String) -> String:
	match stance_effect_type:
		...
		"unicorn_grace": return "Единорог навсегда получает +20 брони и +20 уклонения (сохраняется даже после прерывания стойки)."
		...
		"kikimora_beauty": return "Пока стойка активна, при получении урона кикимора наносит 20% своего урона всем противникам."
		"jester_deadly_strike": return "В начале следующего хода атакует случайного врага на часть его текущего здоровья."
		"indigo_hunt_stance": return "Отступает на 1 позицию при входе в стойку. В начале следующего хода атакует доступную цель на 2-3 позиции с 150% урона."
		...
		"duna_harmony": return "Назад на 1. Союзники навсегда получают иммунитет к дебаффам; отменённый дебафф лечит цель на 10% HP."
...
static func describe_active_effect(effect_id: String, stat: String, value: int, source: String) -> String:
	...
	match key:
		...
		"forget": return "Забытьё: снижает удачу (крит) на 10% за каждый уровень стака."
		...
		"warrior_of_day_armor", "warrior_of_day_evasion": return "Воин дня: +10 максимального HP навсегда, +10 брони и +10 уклонения на 3 хода."
		...
		"mummy_ancient_curse_mark": return "Древнее проклятие: -2 к урону и -2 к броне до конца боя."
		"mummy_death_aura": return "Аура смерти: враги, задетые способностью, получают -15 удачи на 2 хода."
		...
		"koschei_immortal_armor": return "Неубиваемый: при входе в стойку получает +30 брони и метку провокации на 1 ход."
...
static func get_ability_marker_description(marker: String) -> String:
	match marker:
		...
		"titan_mighty_hit": return "Каждое использование даёт титану +5 урона себе навсегда."
		"susanoo_sword_dance": return "За каждое убийство этой способностью Сусаноо лечится на 12% максимального HP."
		...
		"loki_trickster_dance": return "+30 уклонения Локи на 1 ход, вперёд на 1; каждый враг получает -10 удачи на 2 хода; Локи получает +10 удачи за каждого врага на 2 хода."
		"koschei_hoard_gold": return "Снимает с себя дебаффы и восстанавливает 15 фантазии."
		...
		"dwarf_smith_upgrade": return "Цель получает дополнительную броню и атаку на 2 хода в зависимости от жетонов кузнеца (жетоны не расходуются)."
		"chernobog_terrifying_end": return "Если у Чернобога меньше 50% HP, удар не может промахнуться и наносит чистый урон."
		"chernobog_endless_horror": return "При повторном срабатывании эффект обновляется, а не складывается заново."
...
static func get_passive_description(effect_type: String) -> String:
	match effect_type:
		...
		"cannon_position_damage": return "Урон наносится по цепочке целей: 150% первой цели, 120% второй, 90% третьей и далее по убыванию"
		...
		"virgo_buff_amplify": return "Баффы, получаемые союзниками, усиливаются на +100% от изначального значения за каждую живую Деву в команде (накапливается линейно)"
		...
		"banshee_death_debuff": return "При смерти все герои получают +20 к атаке на 1 ход"
		"bird_death_damage": return "При смерти от удара наносит 80% своей атаки убившему"
		...
		"kikimora_extended_debuffs": return "Кикимора: пока жива, дебаффы её команды на героев длятся на 1 ход дольше."
		...
		"mermaid_depth_heal": return "В яростной глубине лечится вместо получения урона (только сама, не распространяется на союзников)"
		"merwarrior_move_luck": return "При перемещении получает +10 удачи на 1 ход."
		...
		"oboroten_round3_power": return "С третьего раунда оборотень навсегда получает +30 уклонения, +30 удачи и +30 урона."
		...
		"triton_enemy_move_armor": return "Пассивка Тритона: когда враг перемещается, этот враг получает -10 брони на 1 ход."
		"troll_self_stun_on_no_damage": return "Если его атака не наносит урон (промах), оглушает себя на 1 ход"
		"utoplennitsa_location_amplify": return "В Топи штраф первой позиции дополнительно увеличивает накопленную потерю инициативы, брони и уклонения."
...
static func get_battle_effect_name(location_id: String) -> String:
	match location_id:
		# Подземелье
		"helheim": return "Хельхейм (туман: -20 к урону героев)"
		"hell": return "Ад (-5 величия, -5 фантазии)"
		"tunnels": return "Тоннели (регенерация при броне >50%)"
		# Небо
		"clouds": return "Облака (враг, выживший после удара, получает +10 уклонения)"
		"stars": return "Звёзды (баффы по позициям врагов)"
		"mountains": return "Горы (Тут не место для малышни)"
		# Цивилизация
		"arena": return "Арена (+20% урона от способностей)"
		"castle": return "Замок (заклинания x1.5, до 2 за раунд на команду)"
		"desert": return "Пустыня (5 чистого урона всем)"
		# Море
		...
		"garden": return "Сад (бог получает +15 урона, если на него наложен дебафф)"
